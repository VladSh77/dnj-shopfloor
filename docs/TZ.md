...
| REQ-002 | WHEN an operator authenticates with a PIN, THEN the system SHALL require a 4-digit PIN, store it as SHA-256 with a unique per-operator salt, allow ≤ 3 wrong attempts / 5 min, and lock the account for 15 min after 3 failures. Correct PIN grants access within 1 s. | Test: успіх/невдача входу, спрацювання блокування після 3 спроб, тривалість блокування; заміри часу відгуку. Inspect: код хешування+солі+lockout. | M | ⚠️ |
...
> Кожен ендпоінт SHALL мати повну OpenAPI 3.x-специфікацію: схеми запиту/відповіді, коди помилок (400/401/403/404/422/500), NFR (латентність, автентифікація). Пороги латентності зведено в NFR-PERF (§3).

| ID | Ендпоінт · EARS-вимога | Критерій приймання / Верифікація | MoSCoW | Статус |
|----|------------------------|----------------------------------|:------:|:------:|
| REQ-035 | `GET /kiosk`: unauthenticated → 401; with valid `kiosk_id` → 200 + JSON `{id,name,location,status,last_sync_time}`; missing/invalid → 400; not found → 404. | Test: усі коди відповіді; типи/довжини полів; p95 < 200 ms. | M | ⚠️ |
| REQ-036 | `POST /dnj_shopfloor/authenticate`: valid creds → 200 + `{access_token(JWT), expires_in:3600}`; invalid → 401 "Invalid credentials"; malformed → 400. Failed attempts logged ≥ 30 днів. | Test (Gherkin): валідні/невалідні креди, валідність JWT, час < 500 ms (99 %). Inspect: лог невдалих спроб. | M | ⚠️ |
| REQ-037 | `POST /dnj_shopfloor/session/open`: valid `device_id`+`user_id` → 200 + `session_id`(UUID) ≤ 300 ms; invalid → 400 ≤ 200 ms; ≤ 1 active session per pair. | Test: створення сесії, валідація, унікальність активної сесії. | M | ✅ |
| REQ-038 | `POST /dnj_shopfloor/session/action`: valid `session_id`+`action_type`(start_work/end_work/pause)+`action_data` → record action, 200 ≤ 300 ms; expired/invalid session → 401 ≤ 250 ms. | Test: типи дій, зміни стану сесії, час відгуку. | M | ✅ |
| REQ-039 | `POST /dnj_shopfloor/session/status`: valid session id + new status → update status, 200 ≤ 200 ms. | Test: оновлення статусу в бекенді, код 200, час відгуку. | M | ✅ |
| REQ-040 | `POST /dnj_shopfloor/workorders`: valid WO payload → create WO, assign id, 201 + id ≤ 200 ms; invalid → 400. | Test: створення WO в БД, повернення id, час відгуку. | M | ✅ |
| REQ-041 | `POST /dnj_shopfloor/workcenters`: valid payload (`name`,`location`,`capacity>0`) → create, 201 + full schema ≤ 250 ms; schema-violation → 400 з деталями. | Test: створення+ретрив GET ≤ 500 мс; валідація схеми. | M | ✅ |
| REQ-042 | `POST /dnj_shopfloor/dashboard`: `{startDate,endDate}`(ISO 8601) → aggregated production metrics, 200 ≤ 500 ms (95 %); invalid dates → 400. | Test: валідні/невалідні запити, дані й коди; перф-тести. Inspect: OpenAPI. | M | ✅ |
| REQ-043 | `POST /dnj_shopfloor/machine/config`: `{machineId(UUID),configuration}` → update config, 200 + confirm ≤ 300 ms, persistent+retrievable; unknown machine → 404. | Test: валідні/невалідні апдейти, персистентність через наступний GET. | M | ✅ |
| REQ-044 | `POST /dnj_shopfloor/machine/heartbeat`: `{machine_id(UUID)[,status]}` → update `last_active_timestamp` (±1 s), 200 ≤ 50 ms; invalid → 400; unknown → 404. Витримує ≥ 100 rps, p90 < 100 ms. | Test: оновлення таймстемпа, коди; Analyze: перф-звіти. | M | ✅ |
| REQ-045 | `POST /dnj_shopfloor/machine/stats`: `{machine_id(UUID),stats_payload}` → record stats, 202 Accepted ≤ 100 ms; invalid → 400; unknown → 404. | Test: збереження даних, код відповіді; Inspect: OpenAPI-схема. | M | ✅ |
...
5. **API-контракти:** згенерувати OpenAPI 3.x для 11 ендпоінтів (§2.6) — зараз специфікації немає.
